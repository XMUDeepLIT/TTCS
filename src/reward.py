...
def compute_reward(data: DataProto, reward_fn: AbstractRewardManager, step: int = 0) -> tuple[torch.Tensor, dict[str, Any]]:
    """
    ...
    """
    try:
        reward_result = reward_fn(data, return_dict=True, step=step)
        reward_tensor = reward_result["reward_tensor"]
        reward_extra_infos_dict = dict(reward_result.get("reward_extra_info", {}))
...
def challenger_compute_reward(data: DataProto, reward_fn: AbstractRewardManager, step: int = 0) -> tuple[torch.Tensor, dict[str, Any]]:
    """
    Compute reward for a batch of data.
    Args:
        data: DataProto object containing the input data.
    ...
    try:
        ...
        topics_infos=reward_result.get('topics_infos',{})
    ...
    return reward_tensor, reward_extra_infos_dict,topics_infos
